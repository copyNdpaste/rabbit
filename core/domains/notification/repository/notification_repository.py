...
from core.domains.notification.dto.notification_dto import NotificationHistoryDto, KeywordTargetUserDto
...
class NotificationRepository:
    ...
    def create_notification_history(self, notification_list: List[NotificationHistoryDto]) -> None:
        try:
    ...
    def get_keyword_target_user(self) -> List[dict]:
        target_user_group = (
            ...
            .with_entities(NotificationModel.user_id, UserNotificationTokenModel.token, KeywordModel.name_1,
                           KeywordModel.name_2, KeywordModel.name_3)
            .join(UserNotificationTokenModel, NotificationModel.user_id == UserNotificationTokenModel.user_id)
            .join(KeywordModel, NotificationModel.user_id == KeywordModel.user_id)
            .filter(NotificationModel.is_available == True, NotificationModel.use_keyword == True)
            .all()
        )
