...
AVAILABLE_SERVICE = ["api", "cron", "worker"]
...
class ECSCompose:
    ...
    def service(self) -> str:
        return f"rabbit-{self.service_type}-{self.environment}"
    ...
    def call_command(self) -> list:
        ...
        if (
            env == "dev"
            or (env == "prod" and service_type == "worker")
            or (env == "prod" and service_type == "cron")
        ):
            return [
                "ecs-cli",
                "compose",
